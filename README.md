...
### 主要工作
#### 网站爬取与数据收集
* 爬取附录中列出的所有网站，特别是其中的文件内容。收集这些文件的数据作为后续处理的基础。
#### 文本分析与关键词提取
* 扫描所有爬取到的文本文件，使用 TF-IDF 算法提取关键词。
* 同时记录每个文件中的高频词，作为后续检索时的辅助信息。
#### 元数据存储与优化
* 将上述提取的元数据（如关键词、高频词）和对应的文件路径存储到 HBase 中，从而加速后续的文本扫描与检索过程。
#### 检索引擎实现
* 基于用户输入的查询（query），结合 文件名、关键词 和 高频词 的相关性，进行排序，并选择出相关性最高的前 top-k 文件。
#### 前端实现与展示
* 完成检索功能的前端实现，展示检索结果，并允许用户查看相关文件或内容。
