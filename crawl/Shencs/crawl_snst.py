...
# 目标 URL 列表
urls = [
    'https://www.snst.ustc.edu.cn/xgzl/list.htm',
    'https://www.snst.ustc.edu.cn/xxwj/list.htm',
    'https://www.snst.ustc.edu.cn/xywj/list.htm'
]

# 基础 URL，用于处理相对路径
base_url = 'https://www.snst.ustc.edu.cn'

# 目标保存路径
save_path = 'cache/Shencs/snst'
...
def parse_article_page(article_url):
    ...
    file_links = soup.select('div.wp_articlecontent a')
    for file_link in file_links:
        file_url = file_link.get("href", "").strip()
        sudyfile_attr = file_link.get("sudyfile-attr", "")
        title = file_link.text.strip()  # 如果没有 sudyfile-attr，则使用链接文本作为标题
        # 如果 sudyfile-attr 存在，优先从中获取标题
        if sudyfile_attr:
